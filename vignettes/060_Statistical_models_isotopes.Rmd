...
CD_global <- read.csv(file.path(here(), "exported_data", "CD_global.csv"))
CD_global %<>% dplyr::select(Code, Species, CDcp, MNNDcp, SDNNDcp)
...
# tableCD <- CD_global %>% dplyr::select(Pond, Species, CDg) %>% pivot_wider(names_from = Species, values_from = CDg)
# write.csv(tableCD, file.path(here(), "exported_data", "tableCD.csv"), row.names = F)
```
...
Correlation <- merge(CD_global %>% dplyr::select(-MNNDg, -SDNNDg), 
                     Pop_amphib_complete %>% 
                       dplyr::select(Pond, Species, NRcp, CRcp, TAcp, SEAc))
head(Correlation)

Metrics <- Correlation %>% dplyr::select(-Pond, -Species) 
dim(Metrics)
...
# Correlation with pearson; with spearman
# Cor with CR > 0.81; 0.67
# Cor with TA > 0.80; 0.76
# Cor with SDNND > 0.79; 0.70
# Cor with SEAc > 0.79; 0.75
# Cor with CDg > 0.81;0.67
# > Keep SEAc and NR
# > D13C was correlated to nothing (max = 0.29)
# > TP was correlated only to CR and SDNND but not the rest (max = 0.15)
...
```{r CDglobal}
#Merge metrics and explicative variables
CDglobal <- merge(CD_global, Candidate, by = "Pond")
print("Is there still 44 populations in the dataset?")
dim(CDglobal)[1] == 44
```
...
```{r check GOF for mean TP, include=FALSE}
# Check residuals
mod1 <- lmer(CDg ~ scale(Canopycover) + (1|Species), na.action="na.fail", data=CDglobal)
plotresid(mod1) #residuals ok
```

```{r check GOF for mean TP transformed, include=FALSE}
# Check residuals
mod1 <- lmer(log(CDg) ~ scale(Canopycover) + (1|Species), na.action="na.fail", data=CDglobal)
plotresid(mod1) #residuals ok
...
```{r dredge TP model, include=FALSE}
# Test effect of candidate variables
mod_CDg <- lmer(log(CDg) ~ scale(Area) + scale(Canopycover) + scale(Aq_VegCover) +
               scale(CrayfishCPUE) +
               scale(Agile_frog) + scale(Tree_frog) + scale(Palmate_newt) + scale(Marbled_newtCPUE) +
               scale(Tadpoles) + (1|Species) #+ (1|Pond)
               , na.action="na.fail", data = CDglobal)
CDg_dredge <- dredge(mod_CDg, rank = AICc, m.lim = c(0,2), extra="R^2")
subset(CDg_dredge, delta < 2)

```

Test R2m vs R2c dans dredge
```{r test R2 conditional or marginal}
mod_CDg <- lmer(log(CDg) ~ scale(Canopycover) + scale(Marbled_newtCPUE) + (1|Species) #+ (1|Pond)
               , na.action="na.fail", data = CDglobal)
r.squaredGLMM(mod_CDg)
```