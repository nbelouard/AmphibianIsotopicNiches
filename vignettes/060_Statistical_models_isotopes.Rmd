...
Envt %<>% filter(Pond != c("I03", "I14"))

plot(Envt$Aq_VegCover ~ Envt$CrayfishCPUE)
anova(lm(Envt$Aq_VegCover ~ Envt$CrayfishCPUE))

Envt_notbest <- Envt %>% filter(!Pond_name %in% c("56", "17_2017", "48", "22", "87", "94"))

plot(Envt_notbest$Aq_VegCover ~ Envt_notbest$CrayfishCPUE)
anova(lm(Envt_notbest$Aq_VegCover ~ Envt_notbest$CrayfishCPUE))

plot(Envt_notbest$Canopycover ~ Envt_notbest$CrayfishCPUE)
anova(lm(Envt_notbest$Canopycover ~ Envt_notbest$CrayfishCPUE))
...
print("Is there still 41 populations in the dataset?")
