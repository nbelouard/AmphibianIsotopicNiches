...
Pop_crayfish <- Pop_metrics %>% filter(Clade == "Crayfish")
...
Pop_crayfish <- merge(Pop_crayfish, Envt, by = "Pond")
dim(Pop_crayfish)[1]
```
...
#First model with crayfish abundance
Model_CCPUE <- lmer(TPavg ~ scale(CrayfishCPUE) + Group, 
                        na.action="na.fail", 
                        data=Pop_dataset)
...
#First model with crayfish presence
Model_C01 <- lm(TPavg ~ as.factor(Crayfish01) + Group, 
                        na.action="na.fail", 
                        data=Pop_dataset)
plotresid(Model_C01)
summary(Model_C01)
