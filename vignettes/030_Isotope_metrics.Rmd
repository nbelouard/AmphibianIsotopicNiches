...
dim(Subset_Dataset[Subset_Dataset$Taxa=="Tree frog",])[1] == 187-19
dim(Subset_Dataset[Subset_Dataset$Taxa=="Palmate newt",])[1] == 209-17
dim(Subset_Dataset[Subset_Dataset$Taxa=="Marbled newt",])[1] == 101
dim(Subset_Dataset[Subset_Dataset$Species=="Crayfish",])[1] == 422-31-43
...
```{r calculate Layman metrics with bootstrapping for populations}
...
```{r calculate average bootstrapped Layman metrics for populations}
...
Pop_metrics %>% group_by(Pond) %>%
  filter(Species != "Crayfish") %>%
  summarise(meanSEAc = mean(SEAc)) %>% 
  arrange(meanSEAc) 
...
Pop_metrics %<>% dplyr::select(Pond, Species, CRcp, NRcp, TAcp, CDcp, MNNDcp, SDNNDcp, SEAc, TPavg, D13Cavg)
...
Calculate Layman metrics - Marbled newt pop vs global
```{r calculate CD for marbled newt}
...
write.csv(CD_global, file.path(here(), "exported_data", "CD_global.csv"), 
          row.names = F)
```
