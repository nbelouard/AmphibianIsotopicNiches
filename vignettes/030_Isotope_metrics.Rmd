...
## a- Layman metrics -- DEPRECATED
Calculate Layman metrics at the regional level
```{r calculate regional Layman metrics}
...
## b- Layman metrics with bootstrapping -- DEPRECATED
Now we calculate the Layman metrics using the bootstrapping method to account for unequal sample sizes. For each pop, we sample 10 individuals in the population, calculate the 6 metrics of Layman, then do it again 999 times.

```{r calculate regional Layman metrics with bootstrapping}
...
```{r summarize average bootstrapped regional Layman metrics}

Regional_metrics_bootstrapped <- as.data.frame(cbind(NRb = colMeans(dNR), 
                                  CRb = colMeans(dCR), 
                                  TAb = colMeans(TA),
                                  CDb = colMeans(CD), 
                                  MNNDb = colMeans(MNND), 
...
## c- SEA metrics -- DEPRECATED
```{r calculate regional ellipses}
...
## d- Merge datasets -- DEPRECATED
Merge datasets, look at the table and export it
```{r merge datasets}

Subset_metrics <- merge(Pop_metrics_Layman, Subset_metrics_ellipses[-1], 
                        by = "Code")
Subset_metrics

write.csv(Subset_metrics, file.path(here(), "exported_data", 
                                    "Regional_metrics.csv"), row.names = F)
```
...
## f- Area of ellipses overlap for regional niches
